...
<div class="container">
  ...
  <?php if(!empty($DATA_TAB)): ?>
      <?php foreach($DATA_TAB as $key => $arr): ?>
        <?php if ( $key > 2 ) { break; } ?>
      ...
      <?php endforeach; ?>
      <?php if (count($DATA_TAB) > 3): ?>
        <div class="row pagination">
          <?php
          $count_page = ceil(count($DATA_TAB) / 3);
          for ($i = 1; $i <= $count_page; $i++):
          ?>
          <button class="btn-sm mr-1 <?php echo ($i == 1) ? 'btn-primary pag-active' : 'btn-light'; ?>"><?php echo $i; ?></button>
          <?php endfor; ?>
